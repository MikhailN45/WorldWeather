...
interface LocationNameProviderUseCase {
    fun execute(location: Location, onLocationNameReceivedCallback: (String) -> Unit)
}

class LocationNameProviderUseCaseImpl @Inject constructor(private val context: Context) :
    LocationNameProviderUseCase {

    override fun execute(location: Location, onLocationNameReceivedCallback: (String) -> Unit) {
        ...
        location.let {
            if (Build.VERSION.SDK_INT >= 33) {
                geoCoder.getFromLocation(it.latitude, it.longitude, 1) { geoList ->
                    ...
                    onLocationNameReceivedCallback(currentCityName)
                }
            } else {
